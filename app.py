...
def check_weather():
    ...
    try:
        ...
        city1_weather, city2_weather = get_daily_weather(city1_code['key'], config), get_daily_weather(
            city2_code['key'], config)
    ...
    result = any([item.is_bad() for item in city1_weather + city2_weather] + [any(w.is_bad() for w in item) for item in
                                                                              stops_weather])
    return render_template('result.html', start=start, end=end, city1_weather=city1_weather,
                           city2_weather=city2_weather, stops=stops, stops_codes=stops_codes,
                           stops_weather=stops_weather, result=result, count=len(stops), start_city=city1_code,
                           end_city=city2_code)
